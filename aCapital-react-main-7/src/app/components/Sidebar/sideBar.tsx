...
const pathEquals = (href: string, pathname: string) =>
  normalizePath(pathname) === normalizePath(href);
const pathNested = (href: string, pathname: string) =>
  pathEquals(href, pathname) || pathname.startsWith(`${normalizePath(href)}/`);
...
export default function Sidebar() {
  ...
  useEffect(() => {
    const sub = user$.subscribe((user) => {
      if (user?.profile) {
        const lastInitial = user.profile.last_name
  ...
  useEffect(() => {
    setOpenGroups((prev) => {
      ...
      menuItems.forEach(({ label, children }) => {
        if (
          children?.some(
            (child) => child.href && pathNested(child.href, pathname)
          )
        ) {
          next[label] = true;
        }
    ...
    <aside className={styles.sidebar}>
      <nav className={styles.menus}>
        <div>
          <Link
            href="/pages/profile"
            className={`${styles.menuItem_profile} ${
              pathname.startsWith("/pages/profile") ? styles.active : ""
            }`}
          >
            <div className={styles.profile}>
              <div className={styles.avatar}>
                ...
                >
                  <path
                    d="M10 0.599609C15.1914 0.599609 19.4004 4.80853 19.4004 10C19.4004 15.1915 15.1914 19.4004 10 19.4004C4.80852 19.4004 0.599612 15.1915 0.599609 10C0.599609 4.80853 4.80852 0.599611 10 0.599609Z"
                    stroke="#B6B7B7"
                    strokeWidth="1.2"
                  />
                  <path
                    d="M10 4.92993C7.93 4.92993 6.25 6.60993 6.25 8.67989C6.25 10.7099 7.84 12.3599 9.95 12.4199C9.98 12.4199 10.02 12.4199 10.04 12.4199C10.06 12.4199 10.09 12.4199 10.11 12.4199C10.12 12.4199 10.13 12.4199 10.13 12.4199C12.15 12.3499 13.74 10.7099 13.75 8.67989C13.75 6.60993 12.07 4.92993 10 4.92993Z"
                    fill="#B6B7B7"
                  />
                  <path
                    d="M16.7807 17.35C15.0007 18.99 12.6207 20 10.0007 20C7.38071 20 5.0007 18.99 3.2207 17.35C3.4607 16.44 4.1107 15.61 5.0607 14.97C7.79071 13.15 12.2307 13.15 14.9407 14.97C15.9007 15.61 16.5407 16.44 16.7807 17.35Z"
                    fill="#B6B7B7"
                  />
                </svg>
              </div>
        ...
        {menuItems.map(({ href, label, icon, children }) => {
          const isActive = href
            ? pathEquals(href, pathname) ||
              pathname.startsWith(`${normalizePath(href)}/`)
            : false;
          const hasChildren = Boolean(children && children.length > 0);
          const isGroupOpen = hasChildren ? Boolean(openGroups[label]) : false;
          let isGroupActive = hasChildren
            ? children!.some((child) => pathNested(child.href!, pathname))
            : false;

          // Для "Обучение" исключаем страницу достижений из активного состояния
          if (
            label === "Обучение" &&
            pathname.startsWith("/pages/education/achievements")
          ) {
            isGroupActive = false;
          }
                    ...
                    >
                      ...
                      <svg
                        ...
                        className={`${styles.arrowIcon} ${
                          isGroupOpen ? styles.arrowIconOpen : ""
                        } ${isGroupActive ? styles.arrowIconActive : ""}`}
                      >
                        <path
